...
@interface KJBannerView() <UICollectionViewDataSource, UICollectionViewDelegate,UICollectionViewDelegateFlowLayout>
...
- (void)setNumberOfItems:(NSInteger)numberOfItems{
    ...
    [self kj_scrollToIndex:index autoScroll:self.autoScroll animated:NO];
}
/// 当前位置
...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section{
    NSInteger count = self.shamItemCount;
    return count;
}
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView
                           cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    if (_numberOfItems == 0 || (_numberOfItems == 1 && (indexPath.row == 0 || indexPath.row == 2))) {
...
- (UICollectionView *)collectionView{
    if (!_collectionView) {
        _collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, 0, 0.000001, 0.000001) collectionViewLayout:self.layout];
